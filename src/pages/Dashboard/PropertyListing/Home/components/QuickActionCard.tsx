...
const QuickActionCard: React.FC<{
...
}> = ({ title, desc, icon, to, onClick, disabled, lockedLabel, showSubscribeButton, onSubscribeClick }) => {
    ...
    const rootClassDisabled =
        'group relative w-full h-full rounded-[15px] border border-[#E2E8F0] bg-white p-[20px] cursor-not-allowed overflow-hidden select-none';
    const handleClick = () => {
        if (disabled) return;
        onClick?.();
    };
    const base = (
        ...
        >
            {(disabled || lockedLabel) && (
                <>
                    {disabled && <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] opacity-60" />}
                    
                    {disabled && showSubscribeButton && (
                        <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white/60 backdrop-blur-sm">
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    e.preventDefault();
                                ...
                                className="px-5 py-2.5 bg-[#0A2D50] text-white rounded-lg font-medium shadow-lg hover:bg-[#072968] active:scale-95 transition-all text-sm flex items-center gap-2 cursor-pointer"
                            >
                                Get Subscription
                    ...
                    {lockedLabel && (
                        <div className={`absolute top-3 right-3 z-10 flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium shadow-sm border ${disabled ? 'bg-white/90 text-[#0A2D50] border-gray-200' : 'bg-orange-50 text-orange-600 border-orange-200'}`}>
                            {typeof lockedLabel === 'string' ? (
                                <>
                                    {disabled ? (
                                        <svg viewBox="0 0 24 24" className="h-3 w-3" stroke="currentColor" fill="none" strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round">
